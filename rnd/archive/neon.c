...
//
// ARCHIVED COPY -- kept for reference; not built. The active runtime
// kernels live in simd/ (dispatched by simd.c at startup based on the
// CPU's ISA tier). This file is the verbatim Apple-Silicon-only NEON
// pack that was used by llm/tensor.c through commit 1614c99
// (2026-05-15). Snapshotted before the simd-dispatcher swap so we
// can fall back if cross-host parity ever drifts. To diff against
// the dispatcher's dotprod tier: `diff rnd/archive/neon.c simd/neon.c`.
//
