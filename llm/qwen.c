// qwen.c -- Qwen3.5-0.8B model implementation (single-file).
//
// `#include`d once from llm.c. Owns the model-specific layers:
//
//   - GGUF parsing (memory-map + KV reader + tensor index)
//   - Byte-level BPE tokenizer (load + encode + decode)
//   - Q4_K_M weight loader and matmul dispatch
//   - KV cache + SSM state cache (Gated DeltaNet)
//   - Forward pass — single-step and batched, attention layers
//     (3,7,11,15,19,23 of 24) and SSM layers (rest)
//   - Sampling primitives (rng, argmax — used by llm.c's sampler
//     chain)
//
// Everything model-agnostic lives in llm.c: the public API
// (llm_create / llm_generate / ...), the <think>/<tool_call>
// state-machine filter, the embedded agent loop, the CLI driver,
// and the sampler chain (apply_rep_penalty / topk / sample_with).
//
// Style: same single-entry/single-exit/no-goto discipline as the
// rest of the project — see ./.agents/memory/coding-philosophy.md.
